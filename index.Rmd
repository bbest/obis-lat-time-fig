...
h = 
  ...
  scale_x_continuous(
    ...
    expand = c(0,0)) +
  scale_y_continuous(
    expand = c(0,0)) + # + scale_x_log10() 
  #scale_y_continuous(trans = mylog_trans(base=10, from=-3), limits=c(0.001,100000))
  scale_fill_discrete(name='depth (m)') +
  labs(x=NULL, y='records x 1000') +
  theme(legend.justification=c(1,0), legend.position=c(1,0)) + 
  coord_flip()

# bin records into 5 deg latitudinal bands
lat_bands = seq(-90, 90, by=5)
h_d = pts@data %>%
  mutate(
    lat_band = cut(lat, lat_bands, include.lowest=T, right=T)) %>%
  group_by(lat_band, bottom_zone) %>%
  summarize(
    records = sum(records))

ggplot(aes(x=lat_band, weight=records, fill=bottom_zone), data=h_d) +
...
ggplot(
  pts@data, aes(x=y_mol, y=records/1000, fill=bottom_zone)
  DF, aes(x=clarity, y=n, fill=cut)) +
